...
int exp_UV_mV[7] = { 0 };
extern unsigned int freq_uv_table[7][3];
...
static ssize_t show_states_enabled_table(struct cpufreq_policy *policy, char *buf) {
	return sprintf(buf, "%d %d %d %d %d %d %d", enabled_freqs[0], enabled_freqs[1], enabled_freqs[2], enabled_freqs[3], enabled_freqs[4], enabled_freqs[5], enabled_freqs[6]);
...
static ssize_t store_states_enabled_table(struct cpufreq_policy *policy, const char *buf, int count) {
	...
	ret = sscanf(buf, "%d %d %d %d %d %d %d", &enabled_freqs[0], &enabled_freqs[1], &enabled_freqs[2], &enabled_freqs[3], &enabled_freqs[4], &enabled_freqs[5], &enabled_freqs[6]);
	if(ret != 1) {
		return -EINVAL;
	}
	else
